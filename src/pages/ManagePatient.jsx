...
const PatientDetailModal = ({ patient, onClose, onUpdate, showToast }) => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      await updateDoc(patientRef, { ...formData, age: calculateAge(formData.dob) });
      onUpdate();
          ...
          <div className="flex-1 p-8 space-y-8 overflow-y-auto border-r border-white/5 custom-scrollbar">
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <DataField label="Patient Name" value={formData.name} isEditing={isEditing} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                <DataField label="Primary Contact" value={formData.phone} isEditing={isEditing} onChange={(e) => setFormData({...formData, phone: e.target.value})} />
                <DataField label="Date of Birth" value={formData.dob} type="date" isEditing={isEditing} onChange={(e) => setFormData({...formData, dob: e.target.value})} />
                <DataField label="Computed Age" value={calculateAge(formData.dob)} isEditing={false} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 tracking-widest">Residential Address</label>
                {isEditing ? (
                  <textarea value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} className="w-full p-4 text-sm text-white border outline-none bg-white/5 border-white/10 rounded-xl focus:border-medical-primary" rows="3"/>
                ) : (
...
const DataField = ({ label, value, isEditing, onChange, type = "text" }) => (
  <div className="space-y-2">
    ...
    {isEditing ? (
      <div className="relative group">
        ...
        <input type={type} value={value} onChange={onChange} className={`w-full ${type === 'date' ? 'pl-10' : 'pl-3'} p-3 text-sm text-white border outline-none bg-white/5 border-white/10 rounded-xl focus:border-medical-primary transition-all`} />
      </div>
    ) : (
