...
const HealthTable = ({ rows, onRowChange, onAdd, onRemove }) => {
  // Define specific health condition options
  ...
  return (
    <div className="p-4 mb-8 glass-panel md:p-6 rounded-2xl">
      <h2 className="pb-2 mb-4 text-xl font-bold border-b border-white/10">Physical Health Conditions</h2>
      ...
      <div className="hidden overflow-x-auto md:block custom-scrollbar">
        <table className="w-full text-left border-collapse">
          <thead className="text-xs text-blue-200 uppercase bg-blue-900/40">
            <tr>
              {['S.No', 'Condition', 'Duration', 'Recent Investigation', 'Remove'].map(h => (
          ...
          <tbody className="divide-y divide-white/5">
            {rows.map((row, index) => (
              <tr key={row.id} className="hover:bg-white/5">
                <td className="p-2 align-top w-14">
                  <TableSNo index={index} />
                </td>
                ...
                <td className="w-20 p-2 pt-3 text-center align-top">
                  <button onClick={() => onRemove(row.id)} className="p-2 text-red-400 hover:text-white">
                    <FaTrash />
                  </button>
                </td>
              </tr>
      ...
      <div className="space-y-4 md:hidden">
         {rows.map((row, index) => (
           <div key={row.id} className="relative p-4 pt-10 space-y-3 border bg-white/5 rounded-xl border-white/10">
             ...
             <button onClick={() => onRemove(row.id)} className="absolute p-2 text-red-400 transition-colors rounded-lg top-2 right-2 hover:text-white hover:bg-red-500/20">
               <FaTrash size={14} />
             </button>
      ...
      <button onClick={onAdd} className="flex items-center gap-2 mt-4 text-sm text-blue-300 hover:text-white">
        <FaPlus className="p-1 text-lg rounded bg-blue-500/20"/> Add Health Condition
      </button>
    </div>
  );
};
